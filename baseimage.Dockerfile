...
ARG CONTAINERD_FUSE_OVERLAYFS_URL="https://github.com/containerd/fuse-overlayfs-snapshotter/releases/download/v1.0.4/containerd-fuse-overlayfs-1.0.4-linux-arm-v7.tar.gz"
...
RUN echo "Installing crictl ..." \
    && curl -sSL --retry 5 --output /tmp/crictl.arm.tgz "${CRICTL_URL}" \
    && tar -C /usr/local/bin -xzvf /tmp/crictl.arm.tgz \

RUN echo "Installing containerd-fuse-overlayfs ..." \
    && curl -sSL --retry 5 --output /tmp/containerd-fuse-overlayfs.arm.tgz "${CONTAINERD_FUSE_OVERLAYFS_URL}" \
    && tar -C /usr/local/bin -xzvf /tmp/containerd-fuse-overlayfs.arm.tgz \
    && rm -rf /tmp/containerd-fuse-overlayfs.arm.tgz
